...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const a = Math.abs(x1 - x2) ** 2;
  const b = Math.abs(y1 - y2) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const mathAtan1 = Math.atan2(x1, y1);
  const mathAtan2 = Math.atan2(x2, y2);
  return Math.abs(mathAtan1 - mathAtan2);
}
...
function getSumOfDigits(num) {
  ...
  const result = number.reduce((acc, elem) => acc + elem, 0);
  return result;
}
...
function getCountOfOddNumbers(number) {
  ...
  for (let i = 1; i <= Math.abs(number); i += 1) {
    if (Math.floor(i / 2) !== i / 2) result += 1;
  }
  return result;
}
